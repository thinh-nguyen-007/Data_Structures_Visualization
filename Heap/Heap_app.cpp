...
void runHeapVisualizer() {
    ...
    if (!backgroundTex.loadFromFile("assets/backgrounds/bg_trees.png")) return;
    if (!font.openFromFile("assets/fonts/Roboto.ttf")) return;
    if (!bolderFont.openFromFile("assets/fonts/RobotoExtraBold.ttf")) return;
    // cursor
