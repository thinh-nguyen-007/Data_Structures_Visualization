...
void Graph::Draw() {
  ...
  for (const auto &edge : edges) {
    ...
    DrawLineEx(startPos, endPos, 4.0f, edge.color);

    // Draw the weight text in the middle of the line
    int midX = (startPos.x + endPos.x) / 2;
    int midY = (startPos.y + endPos.y) / 2;
    DrawText(TextFormat("%d", edge.weight), midX, midY - 20, 20, DARKGRAY);
  }
...
void Graph::LoadFromMatrix(const std::vector<std::vector<int>> &matrix) {
  ...
  // Assign Edges matrix[i][j], always a complete graph
  for (int i = 0; i < n; i++) {
    for (int j = i + 1; j < n; j++) {
...
std::vector<std::vector<int>> Graph::GenerteRandomMatrix(int numVertices) {
  ...
  for (int i = 0; i < numVertices; i++) {
    for (int j = i + 1; j < numVertices; j++) {
      int weight = GetRandomValue(1, 100);
      matrix[i][j] = weight;
      matrix[j][i] = weight;
    }
  }
  ...
  if (fin.is_open()) {
    ...
    for (int i = 0; i < numVertices; i++) {
      for (int j = i + 1; j < numVertices; j++) {
        ...
        } else { // if not a complete graph, return a empty graph
          matrix.clear();
          return matrix;
        }
      }
